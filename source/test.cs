...
public class Setplane : MonoBehaviour
{
    ...
    {
        RecordInit();
        LineViz = GameObject.CreatePrimitive(PrimitiveType.Plane);
        LineViz.transform.localScale = new Vector3(0, 0, 0);
    ...
    void RecordData()
    {
        ...
        else
        {
            ...
            if (Speed.Count > NbSpeedsLine)
            {
                Draw_Line(Pos, Speed, Speed.Count - NbSpeedsLine, Speed.Count - 1);
            }
        }
    ...
    void Draw_Line(List<Vector3> posVec, List<Vector3> speedVec, int startIndex, int endIndex)
    {
        Vector3 speedStart = speedVec[startIndex];
        Vector3  dir, dir_first, pos;
        List<Vector3> dirs = new List<Vector3>();
        for (int i = startIndex; i < endIndex-1; i++)
        {
            dirs.Add(speedVec[i]);
        }
        dir_first = speedVec[startIndex];
        dir_first.Normalize();
        LineMove = Detect_Line(dir_first, dirs);
 
       if (LineMove)
        {
            dir = GetMeanVector(dirs);
            pos = posVec[posVec.Count - 1];

            LineViz.name = "LineViz";
            LineViz.transform.position = new Vector3(pos.x, pos.y, pos.z);
            LineViz.transform.up = dir;
            LineViz.transform.localScale = new Vector3(0.1f, 0.1f, 0.1f);
        }
        else
        {
            LineViz.transform.localScale = new Vector3(0f, 0f, 0f);
        }
    }
    ...
    bool Detect_Line(Vector3 dir_first, List<Vector3> dirs)
    {
        float[] arr_mag = new float[dirs.Count];
        for (int i=0; i< dirs.Count; i++)
        {
            arr_mag[i] = Compute_Vect_Prod(dir_first, dirs[i]).magnitude;
        }
        Array.Sort(arr_mag);
        
        int high_lim = (int)((1 - QuantileLine) * dirs.Count);
        float mean_mag = 0;
